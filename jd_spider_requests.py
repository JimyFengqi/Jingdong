...
class SpiderSession:
    ...
    def __init__(self):
        ...
        print('default user_agent from config file is  %s ' % self.user_agent)
    ...
    def load_cookies_from_local(self):
        ...
        if not os.path.exists(self.cookies_dir_path):
            print('no cookies in local path %s ' % self.cookies_dir_path)
            return False
    ...
    def save_cookies_to_local(self, cookie_file_name):
        ...
        print('save cookies in to local files:  {}{}.cookies'.format(self.cookies_dir_path, cookie_file_name))
...
class QrLogin:
    ...
    def _validate_qrcode_ticket(self, ticket):
        ...
        headers = {
            ...
            'Referer': 'https://passport.jd.com/uc/login?ltype=login',
        }
...
class JdSeckill(object):
    ...
    def make_reserve(self):
        ...
        payload = {'callback': 'fetchJSON', 'sku': 100012043978, }
        headers = {
            'User-Agent': self.user_agent,
            'Referer': 'https://item.jd.com/{}.html'.format(self.sku_id),
        }
        resp = self.session.get(url=url, params=payload, headers=headers)
        logger.info("查询预约时间网页： %s" % resp.url)
        resp_json = parse_json(resp.text)
        if resp_json.get('info') == '预约未开始':
            logger.info('预约未开始, 请您稍后执行, 预约时间段： %s - %s' % (resp_json.get('stime'), resp_json.get('yueEtime')))
            logger.info('抢购时间段： %s - %s' % (resp_json.get('qiangStime'), resp_json.get('qiangEtime')))
            return
        else:
            reserve_url = resp_json.get('url')
            reserve_url = 'https:' + reserve_url
            logger.info("自动执行预约请求， 访问【%s】进行预约" % reserve_url)
            self.timers.start()
            while True:
                try:
                    self.session.get(url=reserve_url)
    ...
    def get_username(self):
        logger.info('starting to get username now')
        """获取用户信息"""
        ...
        resp = self.session.get(url=url, params=payload, headers=headers)
        logger.info("个人信息地址空间： %s" % resp.url)
        try_count = 5
